...
type MintStep = 'metadata' | 'complete-generation' | 'minting' | 'approve' | 'complete' | 'nft-display';
...
export const NFTMintRoadmapModal: React.FC<NFTMintRoadmapModalProps> = ({
  ...
  // Factory completion state
  const [isCompletingGeneration, setIsCompletingGeneration] = useState(false);
  const [generationCompleted, setGenerationCompleted] = useState(false);
  const [factoryTxHash, setFactoryTxHash] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Reset factory completion state
  ...
  useEffect(() => {
    if (selectedSong) {
      ...
      setFormData({
        ...
        image: selectedSong.imageUrl || selectedSong.metadata?.image || '',
        ...
        tags: selectedSong.genre?.join(', ') || selectedSong.metadata?.prompt ||
  ...
  const handleCompleteGeneration = async () => {
    if (!uploadedMetadataCID || !metadataURI) {
      toast.error('Metadata must be uploaded first. Please complete the metadata step.');
      return;
    }
    ...
    try {
      ...
      // Prepare parameters for completeMusicGeneration
      let requestId: number;
      
      // Try to parse selectedSong.id, fallback to timestamp-based ID
      if (selectedSong?.id) {
      ...
      toast.info('Calling completeMusicGeneration...');
      const factoryResult = await factoryMint.completeMusicGeneration(completionParams);

      if (factoryResult.success) {
        toast.success('Music generation completed successfully!');
        setGenerationCompleted(true);
        setFactoryTxHash(factoryResult.hash || 'pending');
        
        // Move to next step
        setCurrentStep('minting');
        setProgress(40);
      } else {
        throw new Error(factoryResult.error || 'Failed to complete music generation');
      }

    } catch (error) {
      console.error('Music generation completion error:', error);
      toast.error(`Failed to complete music generation: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsCompletingGeneration(false);
    }
  };
  ...
  const handleCreateMetadata = async () => {
    ...
    try {
      ...
      // Move to next step
      setCurrentStep('complete-generation');
      setProgress(20);
  ...
  const handleApproveTransaction = async () => {
    if (!mintingCompleted) {
      toast.error('Minting must be completed first.');
      return;
    }
    ...
    try {
      ...
      // Since we're using mintTrack which doesn't require separate approval,
      // this step will just simulate approval completion
      toast.info('Processing approval...');
      
      // Simulate approval process (if needed in the future)
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      setApprovalCompleted(true);
      setApprovalTxHash('approved');
    ...
    if (!selectedSong || !metadataURI || !generationCompleted) {
      toast.error('Generation must be completed first. Please complete previous steps.');
      return;
    }
    ...
    try {
      ...
      // Use mintTrack from NFT contract directly
      const mintResult = await nftManager.handleAction('mint', {
      ...
      toast.loading('Waiting for minting transaction confirmation...', { id: 'mint-wait' });
      
      // Set minting completed state
      setMintingCompleted(true);
      setMintTxHash('pending');
  ...
  const handleCompleteProcess = async () => {
    if (!approvalCompleted) {
      toast.error('Approval must be completed first.');
      return;
    }

    try {
      // Create final NFT data with all transaction details
      const finalTokenId = BigInt(Date.now()).toString();
      ...
      setMintedNFT({
        ...
        song: selectedSong,
        mintedAt: new Date().toISOString(),
        transactionHash: mintTxHash || 'pending',
        contractAddress: CONTRACT_ADDRESSES.HIBEATS_NFT,
        blockNumber: finalTransactionData.blockNumber,
        gasUsed: finalTransactionData.gasUsed,
        factoryTxHash: factoryTxHash,
        approvalTxHash: approvalTxHash
      });

      toast.success('🎉 NFT process completed successfully!');
      ...
      if (onMintSuccess) {
        onMintSuccess(finalTokenId);
      }

    } catch (error) {
      console.error('Complete process error:', error);
      toast.error(`Failed to complete process: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  const renderStepIndicator = () => (
    <div className="flex items-center justify-center mb-8">
      <div className="flex items-center space-x-4 overflow-x-auto max-w-full">
        {[
          { step: 'metadata', label: uploadedMetadataCID ? 'Metadata Ready' : 'Create Metadata', icon: Upload },
          { step: 'complete-generation', label: generationCompleted ? 'Generation Complete' : 'Complete Generation', icon: Zap },
          { step: 'minting', label: mintingCompleted ? 'NFT Minted' : 'Mint NFT', icon: Coins },
          { step: 'approve', label: approvalCompleted ? 'Approved' : 'Approve Transaction', icon: CheckCircle },
          ...
          const isCompleted = (item.step === 'metadata' && uploadedMetadataCID) ||
                             (item.step === 'complete-generation' && generationCompleted) ||
                             (item.step === 'minting' && mintingCompleted) ||
                             (item.step === 'approve' && approvalCompleted) ||
                             (['complete', 'nft-display'].includes(item.step) && 
                              ['complete', 'nft-display'].includes(currentStep));
