...
import { useRewardNotifications } from '@/components/notifications/RewardNotificationProvider';

export function useMusicGeneration() {
  const { address } = useAccount();
  const { showQuickRewardFeedback } = useRewardNotifications();
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      // Show reward notification for music generation
      const rewardAmount = "3.0"; // Base reward amount for music generation
      showQuickRewardFeedback('MUSIC_GENERATION', rewardAmount);
      
      toast.success('Music generation completed!');
    }
  }, [isSuccess, refetchDailyLeft, refetchDailyUsed, showQuickRewardFeedback]);
